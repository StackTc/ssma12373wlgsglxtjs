...
public class YuyueController extends BaseController {
    ...
    public String list() {
        ...
        if(session.getAttribute("cx").equals("职工"))
        {
            where += " AND quyu ='"+session.getAttribute("quyu")+"' ";
        }
        ...
        List<Yuyue> list = service.selectPage(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
    ...
    public String getWhere() {
        ...
        if (!Request.get("leixing").equals("")) {
            where += " AND leixing LIKE '%" + Request.get("leixing") + "%' ";
        }
     ...
     * 添加人列表
     */
    @RequestMapping("/yuyue_list_tianjiaren")
    public String listtianjiaren() {
        ...
        List<Yuyue> list = service.selectPage(example, page, 12);

        request.setAttribute("list", list);
        assign("orderby", order);
        assign("sort", sort);
        assign("where", where);
        return "yuyue_list_tianjiaren";
    }
    ...
    public String insert() {
        ...
        post.setLeixing(Request.getInt("leixing"));
    ...
    public String update() {
        ...
        if (!Request.get("leixing").equals("")) post.setLeixing(Request.getInt("leixing"));
        ...
        if (!Request.get("shijian").equals("")) post.setShijian(Request.get("shijian"));
    ...
    public String delete() {
        ...
        Map map = Query.make("yuyue").find(id);
