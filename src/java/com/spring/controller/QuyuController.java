...
public class QuyuController extends BaseController {
    ...
    @Autowired
    private QuyuService service;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/quyu_list")
    public String list() {
        // 检测是否有登录，没登录则跳转到登录页面
        if (!checkLogin()) {
        ...
        List<Quyu> list = service.selectPage(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
        assign("orderby", order); // 把当前排序结果写进前台
        assign("sort", sort); // 把当前排序结果写进前台
        assign("where", where); // 把当前条件写给前台
        return "quyu_list"; // 使用视图文件：WebRoot\quyu_list.jsp
    }
    ...
    public String getWhere() {
        ...
        if (!Request.get("quyu").equals("")) {
            where += " AND quyu LIKE '%" + Request.get("quyu") + "%' ";
        }
        return where;
    }

    @RequestMapping("/quyu_add")
    public String add() {
        if (!checkLogin()) {
    ...
    @RequestMapping("/quyu_updt")
    public String updt() {
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Quyu mmm = service.find(id);
     ...
     * @return
     */
    @RequestMapping("/quyuinsert")
    public String insert() {
        String tmp = "";
        Quyu post = new Quyu(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
     ...
     *  删除
     */
    @RequestMapping("/quyu_delete")
    public String delete() {
        if (!checkLogin()) {
            return showError("尚未登录");
        }
        int id = Request.getInt("id"); // 根据id 删除某行数据
        Map map = Query.make("quyu").find(id);
