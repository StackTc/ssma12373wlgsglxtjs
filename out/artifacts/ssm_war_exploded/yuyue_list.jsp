            ...
            <div class="pa10 bg-warning">
                <form class="form-inline" id="formSearch" action="?">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control class_quyu12" data-value="${param.quyu}" id="quyu" name="quyu">
                        ...
                        <script>
                            $(".class_quyu12").val($(".class_quyu12").attr("data-value"));
                        </script>
                    </div>
                    <div class="form-group">
                        目的地
                    ...
                    <tbody>
                        <c:set var="i" value="0" /><c:forEach items="${list}" var="map"
                            ...
                            <tr id="${map.id}" pid="">
                                ...
                                <td>${map.huowu}</td>
                                <td>${map.jine}</td>
                                <td><ssm:sql var="mapquyu2" type="find">SELECT quyu,id FROM quyu where id='${map.quyu}'</ssm:sql>${mapquyu2.quyu}</td>
                                <td>${map.mudidi}</td>
                                <td>${map.fahuodi}</td>
                                ...
                                <td>
                                    ...
                                    <c:choose>
                                        <c:when test="${'否' == map.iszf }">
                                            <a href="javascript:window.open('zhifu/index.jsp?id=${map.id}&biao=yuyue&ordersn=${map.bianhao}&zongji=${map.jine}')">  </a>
                                        </c:when></c:choose
                                    >
                                </td>
                                <td align="center">
                                    ...
                                    <c:choose>
                                        <c:when test="${map.iszf == '是'}">
                                            <c:if test="${Query.make('dingdan').where('yuyueid',map.id).count() ==0}">
                                                <a href="dingdan_add.do?id=${map.id}">订单</a>
                                            </c:if>
                                        </c:when>
                                    </c:choose>
